...
export class GeminiService {
  ...
  private async request(payload: Record<string, unknown>): Promise<string | null> {
    ...
    try {
      const res = await fetch(`${GEMINI_API_BASE}/${this.getModel()}:generateContent`, {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) {
        ...
        console.warn(`[gemini] ${res.status} ${res.statusText}: ${body.slice(0, 300)}`);
        return null;
      }
